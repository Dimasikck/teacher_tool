...
def generate_attendance_report(group_id, start_date=None, end_date=None):
    ...
    for student_data in group_analytics['students']:
        report_data.append({
            'Студент': student_data['student'].name,
...
@analytics_bp.route('/analytics/api/category-distribution')
@login_required
def api_category_distribution():
    ...
    if end_date:
        ...
        end_date = datetime.now()
    dist = generate_grade_distribution(group_id, start_date, end_date)
    return jsonify({
        'labels': list(dist.keys()),
        'data': list(dist.values())
    })

@analytics_bp.route('/analytics/api/attendance-by-groups')
@login_required
def api_attendance_by_groups():
    start_date = request.args.get('start_date')
    end_date = request.args.get('end_date')
    if start_date:
        start_date = datetime.strptime(start_date, '%Y-%m-%d')
    ...
    values = []
    for g in groups:
        ga = get_group_analytics(g.id, start_date, end_date)
        labels.append(g.name)
        values.append(ga['avg_attendance'] if ga else 0)
    return jsonify({'labels': labels, 'data': values})

@analytics_bp.route('/analytics/api/problematic-students')
@login_required
def api_problematic_students():
    group_id = request.args.get('group_id', type=int)
    if not group_id:
        return jsonify({'error': 'group_id is required'}), 400
    start_date = request.args.get('start_date')
    ...
    if end_date:
        ...
        end_date = datetime.now()
    items = get_problematic_students(group_id, start_date, end_date)[:10]
    return jsonify([
        {
            ...
            'avg_grade': i['avg_grade'],
            'completion': i['completion']
        } for i in items
    ])

@analytics_bp.route('/analytics/api/top-students')
@login_required
def api_top_students():
    ...
    if end_date:
        ...
        end_date = datetime.now()
    items = get_top_students(group_id, start_date, end_date)[:10]
    return jsonify([
        {
            'student': i['student'].name,
            'attendance': i['attendance'],
...
def chart_data(group_id):
    ...
    if chart_type == 'attendance':
        ...
        data = {
            'labels': [s['student'].name for s in analytics['students']],
            'datasets': [{
                'label': 'Посещаемость (%)',
        ...
        # Сводные показатели по группе для таблицы на странице аналитики
        analytics = get_group_analytics(group_id, start_date, end_date)
        if not analytics:
            return jsonify({'error': 'Нет данных'})
        data = {
            'total_students': analytics['total_students'],
            'avg_attendance': analytics['avg_attendance'],
            'avg_grade': analytics['avg_grade'],
