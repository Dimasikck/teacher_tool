...
def group_subjects():
    ...
    # Всегда добавляем курс группы как дисциплину по умолчанию
    group = Group.query.filter_by(id=group_id, teacher_id=current_user.id).first()
    if group and (group.course or '').strip():
        subjects.add(normalize_subject_name(group.course))
...
def create_control_point():
    ...
    # Разрешаем несколько контрольных точек в один день
